...
struct ASTnode *modify_type(struct ASTnode *tree, int rtype, int op) {
	...
	if (ptrtype(ltype)) {
		// OK is same type on right and not doing a binary op    example : c = &b;
		if (op == 0 && ltype == rtype)
			return (tree);
	}
	...
	if (op == A_ADD || op == A_SUBTRACT) {
		...
		if (inttype(ltype) && ptrtype(rtype)) {
			...
			if (rsize > 1) 
				...
				return (tree);			// making changes to handle the char declaration variable type to generate assembly
		}
	}
