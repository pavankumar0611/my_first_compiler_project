...
static char *do_compile(char *filename) {
	...
	// Change the input file's suffix to .e
	if (O_preprocess) {
		first_stage = alter_suffix(filename, 'e');
		if(first_stage == NULL) {
	...
	if (!O_preprocess && (Outfile = fopen(Outfilename, "w")) == NULL) {
	...
	if (! O_preprocess) {
		...
		pre_processfile = fopen( first_stage, "w");
		if (pre_processfile == NULL){
			printf("Failed to open the %s\n", first_stage);
			fclose(Infile);
			exit(1);
		}

		while (fgets(path, sizeof(path), Infile) != NULL)
			fprintf(pre_processfile , "%s", path);

		fclose(Infile);
		fclose(pre_processfile);
	}
	return (Outfilename);
}
...
static void usage(char *prog) {
	fprintf(stderr, "Usage: %s [-EvcST] [-o outfile] file [file ...]\n", prog);
	fprintf(stderr, "	-E generates pre processed file\n");
