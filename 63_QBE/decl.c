...
static struct symtable *composite_declaration(int type) {
	...
	for (m = m->next; m != NULL; m = m->next) {
		...
		if (type == P_STRUCT) {
			...
			offset = (m->st_posn > offset) ? m->st_posn : offset;
			...
			int value;
			m->st_posn = 0;
			value = cgprimsize(m->type);
			offset = (value > m->st_posn) ? value : m->st_posn;
		}
	}
...
int declaration_list(struct symtable **ctype, int class, int et1, int et2, struct ASTnode **gluetree) {
	...
	*gluetree = NULL;
	...
	while (1) {
		...
		if (sym != NULL && sym->stype == S_FUNCTION) {
			if (class != C_GLOBAL  && class != C_STATIC)
