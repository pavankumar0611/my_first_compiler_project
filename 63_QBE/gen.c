...
static int genDOWHILE( struct ASTnode * n)
{
	...
	r1 = genAST (n->right , Lend , NOLABEL, NOLABEL, n->op);

	r2 = cgloadint(1, P_INT);
	cgcompare_and_jump(A_EQ, r1, r2, Lend, P_INT);
...
static int genSWITCH(struct ASTnode *n) {
	int *caselabel, no_default = 0;
	int Lend, initial_case;
	int Lcode = 0;
	int i, reg, r2, type;
	struct ASTnode *c, *temp;

	// Create an array for the case labels
	caselabel = (int *) malloc((n->a_intvalue + 2) * sizeof(int));
	if (caselabel == NULL)
	...
	// Generate labels for each case including  true and false label
	//jump to label at end label  at last
	for (i = 0, temp = c = n->right; c != NULL; i++, c = c->right)
		caselabel[i] = genlabel();

	if(c == NULL)
		caselabel[i++] = genlabel();	// generate extra label for no default case , make general case
										//ex : switch with only cases and switch with default cases
	...
	//walk the tree to generate the compare jump instructions for all the cases
	for(i = 0 ; temp != NULL; i++, temp = temp->right) {
		...
		if (temp->op != A_DEFAULT) {
			// Jump to the next case if the value doesn't match the case value
			r2 = cgloadint(temp->a_intvalue, type);
			cgcompare_and_jump(A_NE, reg, r2, caselabel[i + 1], type);
	...
	if(!no_default) 		// jump for switch case with no default statements
		cgjump(Lend);

	initial_case= Lcode - (n->a_intvalue);		//variable to keep track of label number
...
int genAST(struct ASTnode *n, int iflabel, int looptoplabel, int loopendlabel, int parentASTop) {
	...
	switch (n->op) {
		...
		case A_ASSIGN:
			...
			switch (n->right->op) {
				case A_IDENT:
					if (n->right->sym->class == C_GLOBAL  || n->right->sym->class == C_EXTERN || n->right->sym->class == C_STATIC)
		...
		case A_SCALE:
			...
			switch (n->a_size) {
				case 2: return(cgshlconst(leftreg, 1, type));
				case 4: return(cgshlconst(leftreg, 2, type));
				case 8: return(cgshlconst(leftreg, 3, type));
				default:
