...
// Return the number of the register
// made changes by pavan  to handle different condition of ldr here switch statements
...
// changes were made to load address of L (label) and store into variable it it pointing to
int cgloadglobstr(int id) {
	// Get a new register
	int r = alloc_register();
	fprintf(Outfile, "\tldr\tr4 ,=L%d\n",id);
	return (r);
}
...
// 0 means no size.
static int psize[] = { 0, 0, 1, 4, 4 , 4, 4, 4 , 4}; 	// nedd to haven type for all the 8 cases 
...
int cgcompare_and_set(int ASTop, int r1, int r2) {
	...
	fprintf(Outfile, "\tand\t%s, %s, #255\n", reglist[r2], reglist[r2]);	//made changes by pavan to support "and" instructions instead of uxtb .since uxtb does not support by processor
	free_register(r1);
	return (r2);
}
