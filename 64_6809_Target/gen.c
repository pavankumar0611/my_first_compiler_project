...
static int genDOWHILE(struct ASTnode *n, struct ASTnode *nleft,
            struct ASTnode *nright) {
  int Lstart, Lend;
  ...
  // Generate the condition code followed
  // by a jump to the end label.
  genAST(nleft, NOREG, NOLABEL, NOLABEL, n->op);
  genfreeregs(NOREG);

  // Generate the compound statement for the body
  genAST(nright, Lend, NOLABEL, NOLABEL, n->op);
  genfreeregs(NOREG);
...
int genAST(struct ASTnode *n, int iflabel, int looptoplabel,
  ...
  case A_DOWHILE:
	special = 1;
	leftreg = genDOWHILE(n , nleft , nright);
	break;
  case A_SWITCH:
    special = 1;
  ...
  if (!special) {
    ...
    case A_GE:
	  ...
	  parentASTop == A_TERNARY || parentASTop == A_LOGAND ||
	  parentASTop == A_LOGOR || parentASTop == A_DOWHILE) {
