...
enum {
	...
	// Other keywords
	T_IF, T_ELSE, T_WHILE, T_FOR, T_DOWHILE, T_RETURN, 
	...
	T_ARROW, T_COLON, T_ELLIPSIS, T_CHARLIT,

	// Misc
	T_FILENAME, T_LINENUM
};
...
enum {
	...
	A_IF, A_WHILE, A_DOWHILE , A_FUNCTION, A_WIDEN, A_RETURN,			// 30
	A_FUNCCALL, A_DEREF, A_ADDR, A_SCALE,				// 36
	A_PREINC, A_PREDEC, A_POSTINC, A_POSTDEC,			// 40
	A_NEGATE, A_INVERT, A_LOGNOT, A_TOBOOL, A_BREAK,		// 44
	A_CONTINUE, A_SWITCH, A_CASE, A_DEFAULT, A_CAST		// 49
};
...
enum {
	P_NONE, P_VOID=16, P_CHAR=32, P_INT=48, P_LONG=64, P_STRUCT=80, P_UNION=96
};
...
#define st_label st_posn		// For string literals, the associated label
	int st_posn;				// For locals, the negative offset
								...
								// the member from the base of the struc
