...
static struct ASTnode *do_while_statement(void)
{
	...
	if ( condAST->op < A_EQ || condAST->op > A_GE)
		condAST = mkastunary(A_TOBOOL , condAST->op , condAST->ctype, condAST , NULL, 0);
	rparen();
	semi();
	//build and return the AST for do while statement
	return ( mkastnode ( A_DOWHILE , P_NONE, NULL, left , NULL , condAST , NULL, 0));
}
...
static struct ASTnode *single_statement(void) {
	...
	switch (Token.token) {
		...
		case T_DOWHILE:
			stmt = do_while_statement(); stmt->linenum = linenum; return (stmt);
		case T_FOR:
