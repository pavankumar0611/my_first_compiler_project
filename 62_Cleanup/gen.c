...
static int genDOWHILE( struct ASTnode * n)
{
	...
	genAST ( n->right , Lend , NOLABEL, NOLABEL, n->op);
	genfreeregs(NOREG);
...
static int gen_logandor(struct ASTnode *n) {
	...
	reg= genAST(n->left, NOLABEL, NOLABEL, NOLABEL, 0);
	cgboolean(reg, n->op, Lfalse);		// This will generate the jump
	genfreeregs(NOREG);
...
int genAST(struct ASTnode *n, int iflabel, int looptoplabel, int loopendlabel, int parentASTop) {
	...
	switch (n->op) {
		...
		case A_SWITCH:
			return (genSWITCH(n));
		case A_TERNARY:
			if (((n->mid->op < A_ADD || n->mid->op > A_DIVIDE)  && (n->mid->op != A_IDENT)) && (n->right->op < A_ADD || n->right->op > A_DIVIDE) && (n->right->op != A_IDENT))
