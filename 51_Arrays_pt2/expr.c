	...
	switch (Token.token) {
		...
		case T_LPAREN:
			...
			if (type == 0)
				...
				n->type = type;  // we don't need a cast node enum , it can done. We just need to update the type
								 // to variable we are narrowing
			return (n);
...
struct ASTnode *prefix(void) {
	...
	switch (Token.token) {
		case T_AMPER:
			...
			if (tree->sym != NULL && tree->sym->stype == S_ARRAY)
...
struct ASTnode *binexpr(int ptp) {
	...
	if (tokentype == T_SEMI || tokentype == T_RPAREN || tokentype == T_RBRACKET || tokentype == T_COMMA || tokentype == T_COLON  || tokentype == T_RBRACE) {
		left->rvalue= 1;
		return(left);
	}
